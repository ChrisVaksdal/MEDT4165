...
def task1(plot=True):
    ...
    def plotSignalNoiseCombined(subfig, time, signal, signalAmplitude, noise,
                                noiseAmplitude, title):
        ...
        addPlot(axes[0],
                x=time,
                y=signal,
                title=f"Signal (A={signalAmplitude})",
                yLabel="Amplitude [A]")
        addPlot(axes[1],
                x=time,
                y=noise,
                title=f"Noise (A={noiseAmplitude})",
                yLabel="Amplitude [A]")
        addPlot(axes[2],
                x=time,
                y=signal + noise,
                title=f"Combined Signal + Noise",
                xLabel="Time [us]",
                yLabel="Amplitude [A]")
    ...
    if plot:
        ...
        for subfig, signal, sAmp, noise, nAmp in zip(subfigs, signals,
                                                     signalAmplitudes, noises,
                                                     noiseAmplitudes):
            plotSignalNoiseCombined(
                subfig, time * 1e6, signal, sAmp, noise, nAmp,
                f"Signal Amplitude: {sAmp}, Noise Amplitude: {nAmp}")

        subfigs[0].suptitle(
            "Equal Signal- and Noise Amplitude\n Signal is somewhat visible",
            fontsize=36)
        subfigs[1].suptitle(
            "Greater Signal Amplitude\nSignal is clearly visible", fontsize=36)
        subfigs[2].suptitle(
            "Greater Noise Amplitude\nSignal is very hard to see", fontsize=36)
...
def task2(time, signals, noises, plot=True):
    ...
    scale = np.sqrt(
        snrNoScaling /
        10**(TARGET_SNR_dB / 10))  # scale = sqrt(SNR_est / 10^(SNR/10))
    ...
    if plot:
        ...
        addPlot(axes,
                x=time * 1e6,
                y=normalizeSignal(signal + scaledNoise),
                title=title,
                xLabel="Time [us]",
                yLabel="Amplitude [A]")
...
def task3(time, signal, noise, plot=True):
    ...
    noisySpectrum, noisyFreqs = calculatePowerSpectrum(
        normalizeSignal(signal + noise))
...
def task4(noisySpectrum, noisyFreqs, plot=True):
    ...
    addPlot(axes,
            x=time * 1e6,
            y=combined,
            title=f"Unfiltered vs. filtered signal",
            xLabel="Time [us]",
            yLabel="Amplitude [A]")
    addPlot(axes, x=time * 1e6, y=filtered)
    axes.legend([
        f"Signal + Noise (SNR: {snrdBNoFilter:.2f}dB)",
    ...
    addPlot(axes,
            x=time * 1e6,
            y=combined,
            title=f"Unfiltered vs. filtered+windowed signal",
            xLabel="Time [us]",
            yLabel="Amplitude [A]")
    addPlot(axes, x=time * 1e6, y=filteredWindowed)
    axes.legend([
        f"Noisy signal (SNR: {snrdBNoFilter:.2f}dB)",
